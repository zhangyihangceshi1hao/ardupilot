...
RGBLed::RGBLed(uint8_t led_off, uint8_t led_bright, uint8_t led_medium,
               uint8_t led_dim)
    : _led_off(led_off),
...
void RGBLed::_set_rgb(uint8_t red, uint8_t green, uint8_t blue) {
    if (red != _red_curr || green != _green_curr || blue != _blue_curr) {
...
uint32_t RGBLed::get_colour_sequence(void) const {
    ...
    if (AP_Notify::flags.armed) {
        ...
        if ((AP_Notify::flags.flight_mode == 5) ||   // loiter mode
            (AP_Notify::flags.flight_mode == 16) ||  // PosHold mode
            (AP_Notify::flags.flight_mode == 3)) {   // auto mode
    ...
    if (AP_Notify::flags.flight_mode == 2) {  // althold mode
        if (AP_Notify::flags.gps_status >= AP_GPS::GPS_OK_FIX_3D_DGPS &&
            ...
            AP_Notify::flags.pre_arm_gps_check) {
            return sequence_althold_mode_disarmd_good_gps;
        }
        else {
            return sequence_althold_mode_disarmd_bad_gps;
        }
    ...
    if ((AP_Notify::flags.flight_mode == 5) ||   // loiter mode
        ...
        if (AP_Notify::flags.gps_status >= AP_GPS::GPS_OK_FIX_3D_DGPS &&
            AP_Notify::flags.pre_arm_gps_check) {
            return sequence_GPS_mode_disarmd_good_dgps;
        } else if (AP_Notify::flags.gps_status >= AP_GPS::GPS_OK_FIX_3D &&
                   AP_Notify::flags.pre_arm_gps_check) {
            return sequence_GPS_mode_disarmd_good_gps;
        } else {
            return sequence_GPS_mode_disarmd_bad_gps;
        }
    }
...
uint32_t RGBLed::get_colour_sequence_traffic_light(void) const {
    if (AP_Notify::flags.initialising) {
        return DEFINE_COLOUR_SEQUENCE(RED, GREEN, BLUE, RED, GREEN, BLUE, RED,
                                      GREEN, BLUE, BLACK);
    }
...
void RGBLed::handle_led_control(const mavlink_message_t& msg) {
    ...
    switch (packet.custom_len) {
        case 4:
            ...
            FALLTHROUGH;
        case 3:
            rgb_control(packet.custom_bytes[0], packet.custom_bytes[1],
                        packet.custom_bytes[2], rate_hz);
            break;
    }
}
#endif
